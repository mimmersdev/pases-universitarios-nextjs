...
CREATE TABLE "cities" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "cities_code_university_id_pk" PRIMARY KEY("code","university_id")
);
--> statement-breakpoint
CREATE TABLE "config" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE "cities" ADD CONSTRAINT "cities_university_id_universities_id_fk" FOREIGN KEY ("university_id") REFERENCES "public"."universities"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "passes" ADD CONSTRAINT "passes_city_id_university_id_cities_code_university_id_fk" FOREIGN KEY ("city_id","university_id") REFERENCES "public"."cities"("code","university_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_passes_city_id" ON "passes" USING btree ("city_id");--> statement-breakpoint
...
CREATE INDEX "idx_passes_currently_studying" ON "passes" USING btree ("currently_studying");--> statement-breakpoint
